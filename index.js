...
const chika = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('Sofyan AMV','red'),color('MrG{108P}*','lime'))
})
	...
	chika.on('message-new', async (amv) => {
		...
		setTimeout( () => {
			...
			switch(command) {
                ...
                if (args.length < 1) return reply('Enable untuk menyalakan, Disable untuk mematikan')
                if (args[0] === 'enable') {
